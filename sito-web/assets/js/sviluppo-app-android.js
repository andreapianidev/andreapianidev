/* sviluppo-app-android.js — reveal + Android App Brief 5-step form + slot fetch */
(function () {
  'use strict';
  ...
  function setupReveal() {
    const targets = document.querySelectorAll('.an-reveal, .an-offer__stack li, .an-guarantee__card, .an-proof__card');
  ...
  function annotateStaggerOrder() {
    document.querySelectorAll('.an-offer__stack li').forEach((el, i) => el.style.setProperty('--i', i));
  }
  ...
  function setupBriefForm() {
    const root = document.getElementById('an-brief');
    if (!root) return;
    const steps = Array.from(root.querySelectorAll('.an-brief__step'));
    const progress = root.querySelectorAll('.an-brief__progress span');
    const backBtn = root.querySelector('.an-brief__back');
    const nextBtn = root.querySelector('.an-brief__next');
    ...
    function setStep(i) {
      ...
      current = i;
      const card = root.querySelector('.an-brief__form');
    ...
    function show(msg, kind) {
      ...
      okEl.className = 'an-brief__success visible ' + (kind === 'err' ? 'err' : 'ok');
    }
    function hide() { okEl.className = 'an-brief__success'; }
    ...
    function readAnswers() {
      ...
      ['app_type', 'target_device', 'timing'].forEach((k) => {
        if (fd.get(k)) out[k] = String(fd.get(k));
      });
      out.google_services = fd.getAll('google_services');
      return out;
    }
    ...
    function submit() {
      ...
      function done() {
        show('✓ Brief Android ricevuto. Ti scrivo io entro 48h con un MVP plan Android native (stack Kotlin + Compose, services Google suggeriti, timeline 8-12 settimane).', 'ok');
